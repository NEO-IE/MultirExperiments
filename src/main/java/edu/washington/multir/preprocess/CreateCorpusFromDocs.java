...
public class CreateCorpusFromDocs {
	...
	Annotation createTestString(String documentString, String docName) throws IOException, InterruptedException {
		...
		for(CoreMap sentence : doc.get(CoreAnnotations.SentencesAnnotation.class)){
			...
			String cjPreprocessedString = CorpusPreprocessing.cjPreprocessSentence(tokenStringBuilder.toString().trim());
...
//				}
	...
	/**
	 * This function takes the newly obtained document string and cleans the paragraphs. 
	 * The code is lifted from CorpusPreprocessing.java
	 * TODO: One of these 2 classes should be deleted
	 * @param documentString
	 * @param docName
	 * @throws IOException
	 */
	void cleanParagraphs(String documentString, String docName) throws IOException {
		List<String> paragraphs = CorpusPreprocessing.cleanDocument(documentString);
		StringBuilder docTextBuilder = new StringBuilder();
		for(String par: paragraphs){
			docTextBuilder.append(par);
